...
def evaluateSummary(summary_selection,videoName,HOMEDATA,user_score):
     ...
     summary_selection=summary_selection[0:nFrames].reshape(-1);
     ...
     summary_indicator = np.clip(summary_selection,0,1)
     summary_indicator = (summary_selection>0).astype(int)
     sum_ind = list(summary_indicator)
     ...
     for userIdx in range(0,nbOfUsers):
         ...
         user_intersection[userIdx] = np.sum(gt_ind*sum_ind)
         # user_union[userIdx]=sum(np.array(map(lambda x: (1 if x>0 else 0),gt_indicator + summary_indicator)));
         user_union[userIdx] = np.sum(np.clip(gt_ind+sum_ind,0,1))
